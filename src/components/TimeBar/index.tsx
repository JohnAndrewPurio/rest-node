  ...
  return (
    <IonRow className="sound-time">
      <IonRow className="time-bar">
        {bars.map(bar => (
          <div className="one-bar" key={bar.start} style={{width: bar.width, left: bar.position}}>
...
const computeWidth = (min: moment.Moment | null, max: moment.Moment | null, start: moment.Moment | null, end: moment.Moment | null) => {
...
const computePosition = (min: moment.Moment | null, max: moment.Moment | null, start: moment.Moment | null) => {
  if (min && max && start) {
    const total = max.diff(min, 'hours', true)
