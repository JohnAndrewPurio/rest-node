  ...
  return (
    <IonRow style={_styles.container}>
      {pages.map((step, index) =>
        index === 0 ? (
          <div onClick={() => jumpStep(index)} style={_styles.circleActive}>
            <IonText style={{ fontSize: '.8rem' }}>1</IonText>
          </div>
        ) : (
          <>
            ...
            <div
              ...
              style={getStyle('circle', index)}
            >
              <IonText style={{ fontSize: '.8rem' }}>{index + 1}</IonText>
            </div>
          </>
        )
      )}
