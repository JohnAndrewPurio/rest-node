...
export const serviceListener = async (zeroconf: ZeroconfOriginal, setAddress: Dispatch<SetStateAction<string>>) => {
    ...
    const subscriptionHandler: (value: ZeroconfResult) => void = ({ action, service }) => {
        ...
        if (!txtRecord['Rest Node by Exist Tribe'])
            return
