...
const TurnOnDevice: FC<Props> = ({ deviceDetails, setDetails, nextStep }) => {
    ...
    const openManual = () => {
        setTimeout(
            () => present('This will open manual', [{ text: 'Nice' }]),
            200
        );
    };

    const next = () => {
        setDetails((prev: newDeviceDetailsType) => ({
            ...prev,
            switchedOn: true,
        }));
    ...
    return (
        <IonRow style={_styles.container}>
            <IonRow style={_styles.upperPart}>
                <IonText style={_styles.bodyText}>
                ...
                <IonButton
                    onClick={openManual}
                    size="small"
                    style={_styles.openManualLink}
                    fill="clear"
                >
                    Open user manual here
                </IonButton>
            </IonRow>
            <IonRow style={_styles.bottomPart}>
                <IonButton onClick={next} style={_styles.nextBtn}>
                    Next
                </IonButton>
            </IonRow>
        </IonRow>
