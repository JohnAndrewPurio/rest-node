...
export class RestnodeService {
    static async getLastValues(): Promise<RestNodeStateType> { 
        ...
        await Storage.set({ key: storage.RED_NODE_STATES, value: JSON.stringify({ bedtime, waketime }) })
    ...
    static async updateValues(data: RestNodeStateType): Promise<RestNodeStateType> {
        const URL = `${BASE_URL}/restnode/event`;
        await axios.post(URL, data.bedtime)
        await axios.post(URL, data.waketime)
        await Storage.set({ key: storage.RED_NODE_STATES, value: JSON.stringify(data) })
        return data
    }
}
